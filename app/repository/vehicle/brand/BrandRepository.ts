...
export default class BrandRepository extends ConnectVehicleSql implements IBrandRepository {
    ...
    public async findByName(brandName: string): Promise<BrandVehicle> {
        return await this.brandRepo.findOne({ "vhc_bran_name": brandName })
       
    }

    public async findByTypeId(typeId: number): Promise<any> {
        return  await this.brandRepo.find({ "vhc_type_id": typeId })
    }

    public async findByOptions(options: any): Promise<any> {
        return  await this.brandRepo.findOne(options)
    }
