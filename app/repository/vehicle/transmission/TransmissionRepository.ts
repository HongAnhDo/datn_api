...
export default class TransmissionRepository extends ConnectVehicleSql implements ITransmissionRepository {
    ...
    public async findByName(name: string): Promise<TrandmissionVehicle> {
        let transmissionUpdate = await this.transmissionRepo.findOne({ "vhc_tms_slug": name })
        return transmissionUpdate;
    }

    public async findByTypeId(vhc_type_id: number): Promise<TrandmissionVehicle[]> {
        let transmission = await this.transmissionRepo.find({ vhc_type_id: vhc_type_id })
        return transmission;
    }
