...
export default class ModelVehicleRepository extends ConnectVehicleSql implements IModelVehicleRepository {
    ...
    public async findByName(modelName: string): Promise<ModelVehicle> {
        let modelVehicleUpdate = await this.modelRepo.findOne({ "vhc_modl_name": modelName })
        return modelVehicleUpdate;
    }

    public async findByBrandId(branId: number): Promise<any> {
        let result = await this.modelRepo.find({ "vhc_bran_id": branId })
        return result;
    }

    public async findByTypeVehicle(brands: Array<number>): Promise<any>{
        let result = await this.modelRepo.find({ "vhc_bran_id": In(brands) })
        return result;
    }
