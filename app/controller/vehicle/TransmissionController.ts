...
export default class TransmissionController {
    ...
    getTransmissioneByName = async (req: Request, res: Response, next: NextFunction) => {
        ...
        var result = await this.transService.findByName(name).catch(err => MyUtil.handleError(err, res))
    ...
    getTransmissionByTypeId = async (req: Request, res: Response, next: NextFunction) => {

        console.log("Received get Transmission vehicle  by  type id==> GET");
        let vhc_type_id = req.query.vhc_type_id;

        var result = await this.transService.findByTypeId(vhc_type_id).catch(err => MyUtil.handleError(err, res))
